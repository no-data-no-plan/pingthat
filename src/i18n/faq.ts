...
export const faqs: Record<string, FAQByLang> = {
  ...
  "site-speed": {
    en: {
      faqs: [
        { question: "Why is this different from Lighthouse or PageSpeed Insights?", answer: "The Chrome User Experience Report (CrUX) is a public BigQuery dataset Google updates monthly with field metrics aggregated from every Chrome user who opted in. Google uses CrUX as the field-data source for Core Web Vitals ranking signals. Lighthouse, by contrast, runs a single synthetic probe from a clean environment — one device, one network, no real users. Both matter for different questions: Lighthouse for what could be faster in a controlled lab, CrUX for what actually is faster for real visitors. The two datasets often disagree (Lighthouse green + CrUX poor), and the CrUX answer is the one Google uses for ranking." },
        { question: "What does 'not enough data' mean?", answer: "CrUX only publishes data for origins and URLs with enough Chrome traffic to aggregate anonymously. Below the publication threshold (Google does not disclose the exact number publicly), the metrics are withheld to prevent single-user inference. If your site shows 'not enough data', run Lighthouse or WebPageTest for synthetic measurements instead." },
        ...
        { question: "When did INP replace FID, and what changed for measuring responsiveness?", answer: "INP (Interaction to Next Paint) became a stable Core Web Vital on 12 March 2024, replacing FID (First Input Delay) which had been a CWV since 2020 (Google Search Central blog announcement May 2023; web.dev migration post). FID measured only the delay before the browser started processing the user's first interaction; INP measures the entire interaction-to-paint path, capturing slow handlers, expensive style recalculations, and layout thrashing — all of which hide from FID. The 'good' threshold is INP <200 ms (vs FID <100 ms). INP is harder to optimise because it measures the worst interaction across the page lifetime, not just the first; sites that passed FID can fail INP if any single click handler is slow. Google Search Console removed FID metrics on 12 March 2024; other tools had a 6-month deprecation window. Transport upgrades (HTTP/2 RFC 7540, HTTP/3 RFC 9114) improve TTFB and resource parallelism but don't directly fix INP — that requires JavaScript optimisation." },
      ],
    },
    es: {
      faqs: [
        { question: "¿En qué se diferencia de Lighthouse o PageSpeed Insights?", answer: "El Chrome User Experience Report (CrUX) es un dataset público de BigQuery que Google actualiza mensualmente con métricas de campo agregadas de cada usuario de Chrome que ha optado. Google usa CrUX como fuente de datos de campo para las señales de ranking de Core Web Vitals. Lighthouse, en contraste, hace una sonda sintética desde un entorno limpio — un dispositivo, una red, sin usuarios reales. Ambos importan para preguntas distintas: Lighthouse para qué podría ser más rápido en un laboratorio controlado, CrUX para lo que realmente lo es para visitantes reales. Los dos datasets suelen discrepar (Lighthouse verde + CrUX pobre), y la respuesta CrUX es la que Google usa para ranking." },
        { question: "¿Qué significa 'datos insuficientes'?", answer: "CrUX solo publica datos de orígenes y URLs con suficiente tráfico Chrome para agregar anónimamente. Por debajo del umbral de publicación (Google no publica el número exacto), las métricas se retienen para evitar inferir por usuario único. Si tu sitio muestra 'datos insuficientes', ejecuta Lighthouse o WebPageTest como alternativa sintética." },
        ...
        { question: "¿Cuándo reemplazó INP a FID y qué cambió para medir capacidad de respuesta?", answer: "INP (Interaction to Next Paint) se convirtió en Core Web Vital estable el 12 de marzo de 2024, reemplazando a FID (First Input Delay) que había sido CWV desde 2020 (anuncio Google Search Central de mayo 2023; post de migración en web.dev). FID medía solo el retraso antes de que el navegador empezara a procesar la primera interacción del usuario; INP mide el camino completo interacción-a-paint, capturando handlers lentos, recálculos de estilo costosos y layout thrashing — todo lo que se oculta a FID. El umbral 'bueno' es INP <200 ms (vs FID <100 ms). INP es más difícil de optimizar porque mide la peor interacción del ciclo de vida de la página, no solo la primera; sitios que pasaban FID pueden fallar INP si cualquier handler de clic es lento. Google Search Console eliminó métricas FID el 12 de marzo de 2024; otras herramientas tuvieron 6 meses de deprecación. Las actualizaciones de transporte (HTTP/2 RFC 7540, HTTP/3 RFC 9114) mejoran TTFB y paralelismo de recursos pero no arreglan INP directamente — eso requiere optimización de JavaScript." },
      ],
    },
  },
  ...
  "is-it-up": {
    en: {
      faqs: [
        { question: "How does this tool actually decide a site is up?", answer: "The tool sends an HTTP request from a Cloudflare edge server in a specific region and inspects the response status code per RFC 9110 §15 (Fielding, Nottingham & Reschke, 2022). A 2xx response means the server processed the request successfully. A 3xx redirects to another URL — the tool follows up to a sensible limit. A 4xx indicates the client made an invalid request (404 not found, 403 forbidden); the server is technically up but rejecting that path. A 5xx is a server-side error (500 internal, 502 bad gateway, 503 service unavailable, 504 gateway timeout) — server is processing but failing. The tool flags 'soft 200' cases where a 200 OK contains an obvious error page in the body — applications often return 200 for client-rendered errors instead of a proper 4xx/5xx, masking real outages from naive checkers." },
        { question: "What's the difference between DNS failure and HTTP failure?", answer: "Different network layers fail differently. DNS resolution happens before the HTTP request: the tool first asks a resolver to translate the hostname to an IP address. RFC 1035 §4.1.1 (Mockapetris, 1987) defines the response codes — RCODE 3 (NXDOMAIN, name error) means the domain doesn't exist or has been deleted; RCODE 2 (SERVFAIL, server failure) means the resolver couldn't process the query (broken DNSSEC chain, authoritative server timeout, etc.). Either DNS error blocks the HTTP request entirely. The tool reports DNS failures separately from HTTP failures because the remediation path differs: NXDOMAIN means the domain registration or DNS zone is gone, SERVFAIL means the nameserver is broken, while a successful resolution that then yields HTTP 5xx points to application-level issues." },
        { question: "Why do response times vary so much across regions?", answer: "Two factors: physical distance and routing quality. Light in single-mode fibre travels at ~c/n where n≈1.47, giving ~5 ms one-way per 1000 km (~10 ms RTT) plus router-hop processing time. A site hosted in Frankfurt responds faster from Madrid than from São Paulo because the round-trip distance is shorter. Routing quality affects this further — peering arrangements, transit ISPs, and BGP policies determine which path packets actually take, which often isn't the shortest geographic route. Anycast deployment (RFC 4786 BCP 126, Abley & Lindqvist, 2006) lets a single IP address advertised in multiple locations route automatically to the nearest edge — Cloudflare, Google's 8.8.8.8, and Fastly typically achieve sub-50 ms response from regions with nearby edge presence (not globally consistent — interior Africa, Pacific islands, etc. still see higher latency). Non-anycast/CDN sites show 100-400 ms inter-region variation depending on origin location and intercontinental routing." },
        { question: "What does HTTP 403 from every checkpoint mean?", answer: "The site is serving requests but actively rejecting them with 403 Forbidden. Three common causes: (1) WAF rules blocking the cloud IPs the tool runs from — Cloudflare, Vercel, Imperva default-deny ranges that include datacentre ASNs; (2) Geo-blocking refusing traffic from certain countries — common for compliance (GDPR, regional licensing) or anti-DDoS; (3) User-Agent filtering blocking 'bot' requests — though this tool sends a real Mozilla-style User-Agent. Actual end-user browsers in residential ISPs often pass through fine, so the apparent 403 is checkpoint-specific not a real outage. Try a different region, or fall back to checking the site from a browser on a residential connection." },
        { question: "Why does this tool show the site as up when my browser can't reach it?", answer: "The tool runs from neutral cloud infrastructure (Cloudflare's global network) which has direct peering to most major carriers and CDNs. Your local connection adds layers the tool doesn't see: your home/office router (firmware bugs, DNS misconfiguration), your ISP (regional outages, peering disputes that don't affect cloud routes), corporate VPN (split tunnel issues, DNS policy), browser cache (serving an outdated error page), browser extensions (ad blockers, security extensions), or operating system DNS resolver. If multiple checkpoints see the site as up but your specific connection cannot, the issue is in your local network path — try `dig` from command line, switch to mobile data, or check from a different network. The tool's verdict 'site is up' answers 'is the public internet reaching it', not 'is your specific connection reaching it'." },
      ],
    },
    es: {
      faqs: [
        { question: "¿Cómo decide la herramienta que un sitio está online?", answer: "La herramienta envía una petición HTTP desde un servidor edge de Cloudflare en una región específica e inspecciona el código de estado de la respuesta según RFC 9110 §15 (Fielding, Nottingham y Reschke, 2022). Una respuesta 2xx significa que el servidor procesó la petición. Una 3xx redirige a otra URL — la herramienta sigue hasta un límite razonable. Una 4xx indica que el cliente hizo una petición inválida (404 no encontrado, 403 prohibido); el servidor está técnicamente online pero rechaza esa ruta. Una 5xx es un error del lado del servidor (500 interno, 502 bad gateway, 503 service unavailable, 504 gateway timeout) — el servidor procesa pero falla. La herramienta detecta casos de 'soft 200' donde un 200 OK contiene una página de error obvia en el cuerpo — las aplicaciones a menudo devuelven 200 para errores renderizados en cliente en vez de un 4xx/5xx correcto, ocultando caídas reales a verificadores ingenuos." },
        { question: "¿Cuál es la diferencia entre fallo DNS y fallo HTTP?", answer: "Capas de red distintas fallan de forma distinta. La resolución DNS ocurre antes de la petición HTTP: la herramienta primero pide a un resolver que traduzca el nombre de host a una dirección IP. RFC 1035 §4.1.1 (Mockapetris, 1987) define los códigos de respuesta — RCODE 3 (NXDOMAIN, name error) significa que el dominio no existe o ha sido borrado; RCODE 2 (SERVFAIL, fallo de servidor) significa que el resolver no pudo procesar la consulta (cadena DNSSEC rota, timeout del servidor autoritativo, etc.). Cualquiera de los dos errores DNS bloquea la petición HTTP por completo. La herramienta reporta fallos DNS separados de fallos HTTP porque la vía de remediación difiere: NXDOMAIN significa que el registro de dominio o la zona DNS desapareció, SERVFAIL significa que el nameserver está roto, mientras que una resolución exitosa que luego da HTTP 5xx apunta a problemas de aplicación." },
        { question: "¿Por qué varían tanto los tiempos de respuesta entre regiones?", answer: "Dos factores: distancia física y calidad de routing. La luz en fibra mono-modo viaja a ~c/n con n≈1,47, dando ~5 ms one-way por 1000 km (~10 ms RTT) más tiempo de procesamiento por salto de router. Un sitio alojado en Frankfurt responde más rápido desde Madrid que desde São Paulo porque la distancia ida y vuelta es menor. La calidad del routing afecta esto aún más — los acuerdos de peering, ISPs de tránsito y políticas BGP determinan qué camino toman realmente los paquetes, que a menudo no es la ruta geográfica más corta. El despliegue Anycast (RFC 4786 BCP 126, Abley y Lindqvist, 2006) permite anunciar una sola dirección IP desde varias localizaciones y enrutar automáticamente al edge más cercano — Cloudflare, el 8.8.8.8 de Google y Fastly suelen lograr respuesta sub-50 ms desde regiones con presencia edge cercana (no globalmente consistente — interior de África, islas del Pacífico, etc. siguen viendo latencia mayor). Los sitios sin anycast ni CDN muestran 100-400 ms de variación inter-región según la ubicación de origen y el routing intercontinental." },
        { question: "¿Qué significa un HTTP 403 desde todos los checkpoints?", answer: "El sitio está sirviendo peticiones pero las rechaza activamente con 403 Forbidden. Tres causas comunes: (1) reglas WAF bloqueando las IPs cloud desde las que corre la herramienta — Cloudflare, Vercel, Imperva tienen rangos por defecto que incluyen ASNs de centros de datos; (2) Geo-bloqueo rechazando tráfico de ciertos países — habitual por cumplimiento (RGPD, licencias regionales) o anti-DDoS; (3) Filtrado por User-Agent bloqueando peticiones 'bot' — aunque esta herramienta envía un User-Agent estilo Mozilla real. Los navegadores de usuarios finales en ISPs residenciales a menudo pasan sin problema, así que el 403 aparente es específico del checkpoint, no una caída real. Prueba otra región, o recurre a comprobar desde un navegador en conexión residencial." },
        { question: "¿Por qué la herramienta muestra el sitio como online cuando mi navegador no puede llegar?", answer: "La herramienta corre desde infraestructura cloud neutral (red global de Cloudflare) que tiene peering directo con la mayoría de operadores y CDNs grandes. Tu conexión local añade capas que la herramienta no ve: tu router doméstico/oficina (bugs de firmware, mala configuración DNS), tu ISP (cortes regionales, disputas de peering que no afectan a rutas cloud), VPN corporativa (split tunnel, política DNS), caché del navegador (sirviendo página de error vieja), extensiones del navegador (ad blockers, extensiones de seguridad) o resolver DNS del sistema operativo. Si múltiples checkpoints ven el sitio online pero tu conexión específica no, el problema está en tu camino de red local — prueba `dig` desde línea de comandos, cambia a datos móviles, o chequea desde otra red. El veredicto 'el sitio está online' responde a '¿está el internet público alcanzándolo?', no a '¿está tu conexión específica alcanzándolo?'." },
      ],
    },
  },
};
