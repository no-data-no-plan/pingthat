...
> = {
  ...
  "site-speed": {
    en: { title: "Site Speed Check — Core Web Vitals Free", description: "Check LCP, INP, CLS, FCP, and TTFB from real Chrome users. Backed by the Chrome UX Report. Free, instant.", seoHeading: "About this tool", seoText: "Enter a URL or origin to fetch its Core Web Vitals from the Chrome User Experience Report. Results are the 75th percentile across real Chrome users over the past 28 days \u2014 the exact dataset Google uses for ranking signals. Lab tests (Lighthouse, WebPageTest) run a single synthetic probe; this tool shows what actual visitors experienced.", seoBlockHeading: "Why real-user data matters", seoBlockText: "Lab-based tools fire one synthetic request from a clean machine on a fast connection. Real users hit slow networks, old devices, crowded CPUs, and third-party cascades. The CrUX dataset aggregates real field metrics at p75, which is the number Google uses to decide whether a page is fast. If your Lighthouse score is great but CrUX is poor, fix the real users, not the benchmark.", seoFeatures: ["LCP, INP, CLS, FCP, TTFB at the 75th percentile", "Mobile and Desktop form factors", "URL-level and origin-level scope", "Real Chrome user data (past 28 days)", "Clear \"not enough data\" state for low-traffic sites"] },
    es: { title: "Velocidad del Sitio \u2014 Core Web Vitals Gratis", description: "Comprueba LCP, INP, CLS, FCP y TTFB desde usuarios reales de Chrome. Usa el Chrome UX Report. Gratis, instant\u00e1neo.", seoHeading: "Sobre esta herramienta", seoText: "Introduce una URL u origen para obtener sus Core Web Vitals del Chrome User Experience Report. Los resultados son el percentil 75 de usuarios reales de Chrome durante los \u00faltimos 28 d\u00edas \u2014 el mismo dataset que Google usa para se\u00f1ales de ranking. Las pruebas de laboratorio (Lighthouse, WebPageTest) hacen una sonda sint\u00e9tica; esta herramienta muestra lo que experimentan los visitantes reales.", seoBlockHeading: "Por qu\u00e9 importan los datos de usuarios reales", seoBlockText: "Las herramientas de laboratorio lanzan una petici\u00f3n sint\u00e9tica desde una m\u00e1quina limpia y conexi\u00f3n r\u00e1pida. Los usuarios reales tienen redes lentas, dispositivos viejos, CPUs saturadas y cascadas de terceros. CrUX agrega m\u00e9tricas reales al percentil 75, que es el n\u00famero que Google usa para decidir si una p\u00e1gina es r\u00e1pida. Si tu puntuaci\u00f3n Lighthouse es buena pero CrUX es pobre, arregla a los usuarios reales, no el benchmark.", seoFeatures: ["LCP, INP, CLS, FCP, TTFB al percentil 75", "Dispositivos m\u00f3vil y escritorio", "\u00c1mbito URL y origen", "Datos reales de Chrome (28 d\u00edas)", "Estado \"datos insuficientes\" claro para sitios de poco tr\u00e1fico"] },
  },
  "resolver-compare": {
    en: { title: "DNS Resolver Comparison Online Free", description: "Compare DNS answers and latency across Cloudflare, Google, AdGuard, and NextDNS. Detect propagation lag and divergent answers. Free, instant.", seoHeading: "About this tool", seoText: "Enter a domain to query it from multiple public DNS resolvers in parallel. You see which resolvers agree on the answer, how long each took, and any divergence that hints at propagation in progress, GeoDNS, or resolver caching issues. A practical replacement for ping-from-N-locations when you care about DNS, not RTT.", seoBlockHeading: "Why resolver comparison matters", seoBlockText: "DNS does not propagate instantly. During a migration or a config change, resolvers cache the old record until their TTL expires. If Cloudflare 1.1.1.1 returns the new IP but Google 8.8.8.8 still returns the old one, your users on Google will still hit the old server. This tool surfaces that state so you can wait out the TTL or flush caches deliberately.", seoFeatures: ["Parallel queries against 4 major public resolvers", "Consistency verdict across resolvers", "Per-resolver latency (colour-coded)", "Supports A, AAAA, MX, NS, TXT, CNAME", "Divergent-answers detection during propagation"] },
    es: { title: "Comparador de Resolvers DNS Online Gratis", description: "Compara respuestas DNS y latencia entre Cloudflare, Google, AdGuard y NextDNS. Detecta retrasos de propagaci\u00f3n y respuestas divergentes. Gratis, instant\u00e1neo.", seoHeading: "Sobre esta herramienta", seoText: "Introduce un dominio para consultarlo a varios resolvers DNS p\u00fablicos en paralelo. Ves qu\u00e9 resolvers coinciden en la respuesta, cu\u00e1nto tard\u00f3 cada uno y cualquier divergencia que indique propagaci\u00f3n en curso, GeoDNS o problemas de cach\u00e9 del resolver. Un reemplazo pr\u00e1ctico del ping-desde-N-ubicaciones cuando lo que importa es el DNS, no el RTT.", seoBlockHeading: "Por qu\u00e9 importa comparar resolvers", seoBlockText: "El DNS no propaga instant\u00e1neamente. Durante una migraci\u00f3n o un cambio de config, los resolvers cachean el registro anterior hasta que su TTL expira. Si Cloudflare 1.1.1.1 devuelve la IP nueva pero Google 8.8.8.8 sigue devolviendo la vieja, tus usuarios con Google seguir\u00e1n cayendo en el servidor viejo. Esta herramienta muestra ese estado para que esperes el TTL o limpies cach\u00e9s a prop\u00f3sito.", seoFeatures: ["Consultas paralelas a 4 resolvers p\u00fablicos mayores", "Veredicto de consistencia entre resolvers", "Latencia por resolver (con c\u00f3digo de color)", "Soporta A, AAAA, MX, NS, TXT, CNAME", "Detecci\u00f3n de respuestas divergentes durante propagaci\u00f3n"] },
  },
  "reverse-dns": {
    en: { title: "Reverse DNS Lookup Online Free", description: "Resolve PTR records for any IPv4 or IPv6 address. Free, instant, no signup.", seoHeading: "About this tool", seoText: "Enter an IPv4 or IPv6 address to resolve its PTR record \u2014 the reverse DNS name assigned to that IP. Useful for mail server audits, abuse tracing, CDN identification, and understanding which hostname the IP owner publishes.", seoBlockHeading: "Why reverse DNS matters", seoBlockText: "PTR records map an IP back to a hostname. Mail servers check PTR for spam filtering \u2014 missing or mismatched PTR is a strong negative signal for deliverability. Reverse DNS is also useful for identifying what lives behind a given IP when auditing a network or investigating an incident.", seoFeatures: ["IPv4 and IPv6 support", "Full RFC-compliant in-addr.arpa and ip6.arpa construction", "Powered by Cloudflare DoH", "No signup, no logging", "Clear \"no PTR\" state for IPs without reverse DNS"] },
    es: { title: "Consulta DNS Inversa (PTR) Online Gratis", description: "Resuelve registros PTR para cualquier direcci\u00f3n IPv4 o IPv6. Gratis, instant\u00e1neo, sin registro.", seoHeading: "Sobre esta herramienta", seoText: "Introduce una direcci\u00f3n IPv4 o IPv6 para resolver su registro PTR \u2014 el nombre DNS inverso asignado a esa IP. \u00datil para auditar servidores de correo, rastrear abuso, identificar CDNs y entender qu\u00e9 hostname publica el propietario de la IP.", seoBlockHeading: "Por qu\u00e9 importa el DNS inverso", seoBlockText: "Los registros PTR mapean una IP a un hostname. Los servidores de correo consultan PTR para filtrar spam \u2014 un PTR ausente o incoherente es una se\u00f1al negativa fuerte para la entregabilidad. El DNS inverso tambi\u00e9n sirve para identificar qu\u00e9 hay detr\u00e1s de una IP cuando auditas una red o investigas un incidente.", seoFeatures: ["Soporte IPv4 e IPv6", "Construcci\u00f3n completa y conforme al RFC de in-addr.arpa e ip6.arpa", "Potenciado por Cloudflare DoH", "Sin registro ni logs", "Estado \"sin PTR\" claro para IPs sin DNS inverso"] },
  },
  "dnssec-check": {
